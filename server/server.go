...
func (s *Server) Start() error {
	...
	if s.cfg.FallbackTarget != "" {
		if err := handler.SetFallbackTarget(s.cfg.FallbackTarget, s.cfg.FallbackPreserveHost, s.cfg.FallbackRewriteContent); err != nil {
			return fmt.Errorf("fallback target: %w", err)
		}
		log.Info("[SERVER] fallback target configured", "target", s.cfg.FallbackTarget, "preserve_host", s.cfg.FallbackPreserveHost, "rewrite_content", s.cfg.FallbackRewriteContent)
	}
