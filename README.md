...
* `enable_tun2socks`：在本机建立全局透明代理（TUN 网卡 + 路由表），让**经过这台主机的流量**
  按域名/GeoIP 分流后走代理。
* `enable_forward_dns`：在本机 **`0.0.0.0:53`（UDP）** 上提供 DNS 转发服务，供 **LAN 设备**
  当解析器使用。它只做转发，不做屏蔽/缓存策略；LAN 设备的解析结果由它代查后返回。
...
1. **把 LAN 侧（DHCP 下发）的 DNS 指向这台路由器的 LAN IP**，例如 `192.168.1.1`。
   ...
   NAT/透明重定向规则）。
2. 放行 UDP 53（`0.0.0.0:53` 对所有网卡开放，**不要在公网侧放行**，否则就是一个开放解析器）。
