...
* 基于 HTTP/2 传输层，内建连接池与流量整形，大幅降低请求延迟
* 支持两种配置模式：简化模式（兼容 v2 扁平格式，开箱即用）和完整模式（v3 嵌套结构，支持多服务器等高级功能）
* 可配置多服务器切换; 自定义直连白名单(IP/域名)
* 支持系统托盘图标管理 (thanks [systray](https://github.com/getlantern/systray))
* 基于TLS加密传输，上层支持AEAD类型高强度加密通信, 如aes-256-gcm, chacha20-poly1305
...
- **完整模式**：嵌套 JSON 格式，支持多服务器等高级功能，通过 `config_version: 3` 标识
...
- 包含 `"config_version": 3` 且 `servers` 非空 → **完整模式**
- 不包含 `config_version` 或 `servers` 为空 → **简化模式**（自动迁移到完整模式）
...
在完整模式中，将 `servers[].address` 配置为优选IP，`servers[].sni` 配置为对应的域名。
