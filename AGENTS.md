...
client/             客户端核心：Client 结构体 + LatencyTracker
...
11. **被动 RTT 测量**：`client.LatencyTracker` 使用 EWMA 从代理流量的读写时间戳中估算 RTT，替代了主动 ping；支持配置 `latency_offset_ms` 补偿网络抖动
12. **NextProxy 动态学习**：`server/nextproxy` 从 DNS 响应中自动提取 CNAME 目标域名和解析 IP，动态加入代理列表，无需预配置完整域名
13. **v2→v3 配置迁移**：`client/config/migrate.go` 自动检测 v2 格式配置文件并通过 `MigrateV2Config()` 转换为 v3 格式，迁移后备份原文件为 `.bak`
